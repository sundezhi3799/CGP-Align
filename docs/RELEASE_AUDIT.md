...
- The numerical-summary package uses the August 31 snapshot. September hidden-neighbour candidate data are separate.
...
6. **Figure 3H / manuscript mismatch.** The current Word manuscript still describes paired gains. September's graphic uses hidden phenotype neighbours. Decide the final revision and update prose, legend, source dictionary, plotting code and archive together. Do not overwrite the archived paired-gain data.
