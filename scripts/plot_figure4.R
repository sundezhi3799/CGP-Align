...
source_dir <- "assets/figure4"
args <- commandArgs(trailingOnly=TRUE)
if(length(args)!=1) stop("Usage: Rscript scripts/plot_figure4.R <output directory>")
figure_dir <- args[1]
...
plot_b <- ggplot(lift_df, aes(y = name, x = mean_lift50)) +
  ...
  geom_segment(aes(x = 1, xend = mean_lift50, yend = name), linewidth = 0.65, colour = "#2A9D8F", alpha = 0.92) +
  geom_point(size = 2.1, colour = "#138A7E") +
...
plot_c <- ggplot(classifier_long, aes(x = method, y = AUPRC, group = name)) +
  ...
  geom_line(data = classifier_mean, aes(x = method, y = AUPRC, group = name), colour = "#7E5AA6", linewidth = 0.70) +
  geom_point(data = classifier_mean, aes(x = method, y = AUPRC), colour = "#7E5AA6", size = 2.1) +
  geom_text(
...
plot_d <- ggplot(gain_df, aes(y = y_pos, x = gain, colour = metric)) +
  ...
  geom_segment(aes(x = 0, xend = gain, yend = y_pos), linewidth = 0.55, alpha = 0.92) +
...
plot_e_base <- ggplot() +
  ...
  guides(
    ...
    colour = guide_legend(order = 2, nrow=2, byrow=TRUE, override.aes = list(linewidth = 0.75, alpha = 1))
  ) +
...
plot_e <- plot_e_base + ggrepel::geom_label_repel(data = highlight_points,
 aes(x=pca_x,y=pca_y,label=label), colour=ink, size=2.2, family=font_family,
 fill="white", linewidth=0.08, label.padding=unit(0.06,"lines"), label.r=unit(0.03,"lines"), box.padding=0.35, point.padding=0.15,
 max.overlaps=Inf, seed=41, min.segment.length=0, segment.colour="#AAB7C1")

legend_plot <- plot_e + guides(colour=guide_legend(order=2,nrow=1,override.aes=list(linewidth=.75,alpha=1))) +
 theme(legend.direction="horizontal",legend.position="bottom",legend.box="vertical",legend.spacing.y=unit(1,"pt"),legend.margin=margin(0,0,0,0),legend.box.spacing=unit(0,"pt"))
legend_gtable <- ggplotGrob(legend_plot)
legend_ids <- grep("guide-box",legend_gtable$layout$name)
legend_ids <- legend_ids[!vapply(legend_gtable$grobs[legend_ids],inherits,logical(1),"zeroGrob")]
legend_panel <- wrap_elements(full=legend_gtable$grobs[[legend_ids[1]]])
plot_e <- plot_e + theme(legend.position="none")
layout_design <- "AABC\nAAEE\nDDEE\nFFFF"
figure4 <- wrap_plots(A=plot_a,B=plot_b,C=plot_c,D=plot_d,E=plot_e,F=legend_panel,design=layout_design) +
 plot_layout(widths=c(1.15,1.15,1,1.25),heights=c(.85,.65,.95,.23)) &
 theme(plot.background=element_rect(fill="white",colour=NA))
out_base <- file.path(figure_dir,"figure4")
for(ext in c("pdf","svg","png","tiff")) {
 path <- paste0(out_base,".",ext)
 if(ext=="pdf") ggsave(path,figure4,width=183,height=175,units="mm",device=cairo_pdf)
 else if(ext=="svg") ggsave(path,figure4,width=183,height=175,units="mm",device=svglite::svglite)
 else if(ext=="tiff") ggsave(path,figure4,width=183,height=175,units="mm",dpi=600,compression="lzw",bg="white")
 else ggsave(path,figure4,width=183,height=175,units="mm",dpi=600,bg="white")
}
cat("Exported balanced Figure 4\n")
