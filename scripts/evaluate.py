"""Evaluate released checkpoints on their matching prepared test data using the provided model implementation."""
import argparse
import json
import os
...
def main():
    ...
    a = p.parse_args()
    marker = ROOT / 'cgp_align/models/version.json'
    if not marker.exists() or json.loads(marker.read_text())['version'] != 'final_architecture_20260920':
        p.error('Model configuration is missing; run from a complete CGP-Align checkout.')
    from config_args import command_from_config
