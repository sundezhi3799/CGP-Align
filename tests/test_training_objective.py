        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for seed in (31, 37, 41):
                with self.subTest(seed=seed):
                    source = ROOT / f'configs/seed{seed}/joint/configuration.json'
                    target = base / f'inputs/models/seed{seed}/joint/configuration.json'
                    target.parent.mkdir(parents=True)
                    shutil.copyfile(source, target)
                    output = base / f'output{seed}'
                    result = subprocess.run([
                        sys.executable, str(ROOT / 'scripts/train.py'),
                        '--artifacts', str(base / 'inputs'), '--seed', str(seed),
                        '--stage', 'joint', '--output', str(output), '--dry-run',
                    ], capture_output=True, text=True)
                    self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                    records = json.loads((output / 'commands.json').read_text())
                    self.assertEqual(len(records), 1)
                    command = records[0]['command']
                    self.assertEqual(Path(command[2]), ROOT / 'scripts/train_cgp_align_replicate.py')
                    for flag, value in [('--gene_branch_loss_reduction', 'sum'),
                                        ('--compound_loss_weight', '1.0'),
                                        ('--gene_loss_weight', '1.0'),
                                        ('--cg_teacher_weight', '0.0')]:
