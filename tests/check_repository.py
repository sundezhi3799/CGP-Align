...
MANIFEST = ROOT / 'tests/metadata/release_checksums.json'
...
def files():
    for p in sorted(ROOT.rglob('*')):
        ...
        if any(x in {'.git', '.venv', '__pycache__', '.pytest_cache'} for x in rel.parts):
            continue
        if rel.parts[0] in {'output', 'outputs', 'data', 'checkpoints'}:
            continue
...
def main():
    ...
    with (ROOT / 'tests/metadata/source_provenance.csv').open(encoding='utf-8', newline='') as f:
