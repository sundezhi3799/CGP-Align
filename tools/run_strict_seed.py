...
def main():
    ...
    state = dict(status='preprocessing', controller_pid=os.getpid(), seed=a.seed, started=time.time(), jobs=[], code_directory=str(ROOT))
    children = []
    def save():
        tmp = job / 'status.tmp'
    ...
    try:
        ...
        for kind, family, raw in [('compound', 'compound_raw_common3180_v1_20260604_0155', 'raw_common3180_compound'),
            ...
            run([sys.executable, '-u', str(ROOT / 'tools/prepare_strict_profiles.py'),
                 '--raw-dir', str(base / family / raw), '--split-dir', str(base / f'raw3180_seeded_split_3seed_20260605/seed{a.seed}' / (kind + '_source_zscore_plate_well_center_3180')),
                 '--kind', kind, '--seed', str(a.seed), '--output-dir', str(target)], job / (kind + '_preprocessing.log'))
            audit = json.loads((target / 'profile_correction_audit.json').read_text())
            if audit['seed'] != a.seed or audit['fit_val_overlap'] or audit['fit_test_overlap']:
        ...
        for branch, gpu in zip(('compound', 'orf', 'crispr'), gpus):
            cfg = json.loads((ROOT / 'manifests/checkpoint_configs' / (f'initializer_seed{a.seed}_' + branch + '.json')).read_text())
            if cfg['seed'] != a.seed: raise ValueError('Branch recipe seed mismatch')
            ...
            cfg.update(data_dir=str(dirs['compound' if branch == 'compound' else 'gene']),
                       run_name=f'strict_seed{a.seed}_' + branch, checkpoint_dir=str(branch_root / 'checkpoints'),
        ...
        while True:
            ...
            state['branch_exit_codes'] = codes; save()
            if all(code == 0 for code in codes): break
            time.sleep(30)
        for path in checkpoints.values():
            if not path.is_file(): raise FileNotFoundError(path)
        cfg = json.loads((ROOT / f'manifests/checkpoint_configs/primary_seed{a.seed}.json').read_text())
        if cfg['seed'] != a.seed: raise ValueError('Joint recipe seed mismatch')
        joint = job / 'joint'; joint.mkdir()
        cfg.update(compound_data_dir=str(dirs['compound']), gene_data_dir=str(dirs['gene']),
                   gene_protein_embedding_dir=str(protein), run_name=f'strict_seed{a.seed}_joint',
